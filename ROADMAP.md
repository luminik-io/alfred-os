...
- **Native local client v2.** Slack remains the primary planning and collaboration UI. The native Mac/Windows client should wrap Alfred's local control surface for install checks, health, logs, credentials, memory review, safe pause/resume, dry-run launch, and recovery. Keep the workflows available through `alfred serve` and CLI first so the client stays a thin shell rather than a second runtime. IC: core. Effort: M. Issue: TBD.
