...
Wizard order (each step is idempotent — re-running won't duplicate):
    ...
    4. AWS (optional)  — per-agent IAM profiles for Huntress / Gordon /
                         Oracle if the operator wants them.
...
def discover_agents(bin_dir: Path) -> list[str]:
    ...
    A runner is "present" when bin/<codename>.py exists. Order is the
...
def render_agents_conf(state: WizardState) -> str:
    ...
    (label, script, schedule, needs_java, log_stem). Includes a header
    ...
    lines = [
        ...
        "#",
        "# label\tscript\tschedule\tneeds_java\tlog_stem",
        "",
    ]
    for role in state.enabled_roles:
        ...
        elif (Path("bin") / f"{codename}.py").as_posix() and codename != "fleet-recap":
            script = f"{codename}.py"
            log_stem = f"alfred.{codename}"
        else:
            script = f"{codename}.py"
            log_stem = f"alfred.{codename}"
        label = f"alfred.{codename}"
        lines.append(f"{label}\t{script}\t{schedule}\tno\t{log_stem}")
    return "\n".join(lines) + "\n"
...
def env_assignments_for(state: WizardState) -> dict[str, str]:
    ...
    for role in state.enabled_roles:
        ...
        if repos:
            out[f"ALFRED_{codename.upper().replace('-', '_')}_REPOS"] = ",".join(repos)
...
def step_12_smoke(state: WizardState) -> None:
    ...
    print("    alfred status        — fleet health snapshot")
    print("    alfred logs <agent>  — per-agent logs")
    print("    alfred pause <agent> — pause an agent")
    print("    alfred resume <agent> — resume")
    print()
