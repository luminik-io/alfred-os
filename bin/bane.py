...
def main() -> int:
    ...
    if not new_commits:
        remove_worktree(repo, wt)
        msg = f"[{AGENT.upper()}-NO-COMMIT] {repo}: {engine_used} success but no commit. turns={result.num_turns}"
        print(msg)
        slack_post(msg)
...
""")
    ...
    if pr_url:
        ...
        msg = (
            f"✅ {AGENT.title()} {repo}: {pr_url} (engine={engine_used}, turns={result.num_turns})"
        )
        print(msg)
        slack_post(msg)
