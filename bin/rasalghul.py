...
from agent_runner import (  # noqa: E402
    GH_ORG, WORKSPACE, WORKSPACE_ROOT,
    EventLog, PreflightFailed, PreflightSpec,
    SpendState, claude_invoke, doctor_mode, gh_json, gh_pr_comment,
    is_globally_blocked, is_repo_paused, preflight, run, short, slack_post, with_lock,
)
...
REVIEW_REPOS = [
    r.strip()
    for r in os.environ.get("ALFRED_RASALGHUL_REPOS", "").split(",")
    if r.strip()
]
...
SPECS_REPOS = {
    r.strip()
    for r in os.environ.get("ALFRED_RASALGHUL_SPECS_REPOS", "").split(",")
    if r.strip()
}
...
def pick_pr() -> tuple[str, dict] | tuple[None, None]:
    ...
    for repo in REVIEW_REPOS:
        ...
        prs = gh_json([
            ...
            "--json", "number,title,headRefName,url,createdAt,labels,isDraft",
            "--limit", "30",
        ], default=[])
        if not prs:
            continue
        for pr in prs:
            ...
            state = gh_json(["gh", "pr", "view", str(pr["number"]), "-R", f"{GH_ORG}/{repo}",
                              "--json", "state"], default={}).get("state")
            if state != "OPEN":
                continue
            ...
            view = gh_json([
                "gh", "pr", "view", str(pr["number"]), "-R", f"{GH_ORG}/{repo}",
                "--json", "comments,commits"
            ], default={"comments": [], "commits": []})
            comments = view.get("comments", []) or []
            commits = view.get("commits", []) or []

            our_reviews = [c for c in comments
                           if c.get("body", "").startswith(REVIEW_AUTHOR_PREFIX)]
            if not our_reviews:
                return repo, pr

            try:
                last_review_ts = max(
                    datetime.strptime(c["createdAt"], "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
                    for c in our_reviews if c.get("createdAt")
                )
...
def main() -> int:
    ...
    if spend.state.get("reviews_posted", 0) >= DAILY_REVIEW_CAP:
        print(f"[{AGENT.upper()}-REVIEW-CAP] {spend.state['reviews_posted']} reviews posted today. Skipping.")
    ...
    meta = gh_json(["gh", "pr", "view", str(pr_num), "-R", f"{GH_ORG}/{repo}",
                     "--json", "title,body,additions,deletions"], default={})
...
"""
    ...
    result = claude_invoke(
        prompt, workdir=local_path, allowed_tools="Read,Bash,Glob,Grep",
    ...
    if not result.success:
        ...
        msg = f"❌ {AGENT.title()}: subtype={result.subtype} turns={result.num_turns} on PR {pr_num}"
        print(msg); slack_post(msg)
    ...
    if not text.startswith(REVIEW_AUTHOR_PREFIX):
        ...
        if idx >= 0:
            ...
            text = text[idx:]
            if preface:
                print(f"[{AGENT.upper()}-SALVAGED] stripped {len(preface)}-char preface", file=sys.stderr)
        else:
            print(f"[{AGENT.upper()}-SALVAGED] wrapping {len(text)}-char output under synthetic header", file=sys.stderr)
            text = (
    ...
    state = gh_json(["gh", "pr", "view", str(pr_num), "-R", f"{GH_ORG}/{repo}",
                     "--json", "state"], default={}).get("state")
    if state != "OPEN":
    ...
    events.emit("review_posted", repo=f"{GH_ORG}/{repo}", number=pr_num,
                turns=result.num_turns, p0_count=len(p0_findings),
                p1_count=len(p1_findings), split_comments=posted_split)
