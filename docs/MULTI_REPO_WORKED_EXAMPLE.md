...
usually means the specs or planning repo, not one of the implementation repos.
The issue carries `agent:large-feature`; Batman derives the bundle label for
the child issues from the title.
...
automerge. After every child issue is filed, Batman marks and closes the
original `agent:large-feature` issue so the same parent cannot be picked up
twice. The bundle is tracked by the `agent:bundle:add-org-slug` label that
every child carries.
...
- [`SPECS_DRIVEN_DEVELOPMENT.md`](SPECS_DRIVEN_DEVELOPMENT.md): writing the
  parent `agent:large-feature` issue.
- [`OUTPUT_SAMPLES.md`](OUTPUT_SAMPLES.md): every output shape that appears
  in the trace above, in one place.
