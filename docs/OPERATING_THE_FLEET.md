...
If Claude reports unauthenticated, run `claude auth login`. The auth blob at `~/.claude/` expires rarely but does expire. The hybrid agents will have been failing over to Codex; pure-Claude agents will have been silent. See [Engine routing](./ENGINE_ROUTING.md).
