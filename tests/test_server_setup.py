...
def test_install_inventory_ignores_unreadable_incidental_alfred_label(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
    ...
    launch_agents = home / "Library" / "LaunchAgents"
    launch_agents.mkdir(parents=True)
    (launch_agents / "com.vendor.alfred.helper.plist").write_text(
        """<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN"
 "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
  <key>Label</key>
  <string>com.vendor.alfred.helper</string>
  <key>ProgramArguments</key>
  <array>
    <string>/usr/bin/true</string>
  </array>
...
def test_install_inventory_detects_single_legacy_engineering_launcher_without_prefix_opt_in(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
    runtime = tmp_path / "alfred"
    old_runtime = tmp_path / "legacy" / "eng"
    label = ".".join(("legacy", "eng", "worker"))
    (home / "Library" / "LaunchAgents").mkdir(parents=True)
    monkeypatch.setenv("HOME", str(home))
    monkeypatch.setenv("ALFRED_HOME", str(runtime))
    monkeypatch.delenv("ALFRED_REPO", raising=False)
    ...
    def fake_program_args(probed_label: str, _env: dict[str, str]) -> list[str]:
        if probed_label == label:
            return [str(old_runtime / "bin" / "agent-launch")]
        return []
    ...
    assert inventory["unmanaged_scheduler_jobs"] == [label]
    assert inventory["unmanaged_scheduler_count"] == 1
...
def test_install_inventory_ignores_external_agent_launch_with_alfred_substring(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
...
def test_install_inventory_ignores_external_alfred_path_with_incidental_label(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
    ...
    def fake_program_args(label: str, _env: dict[str, str]) -> list[str]:
        if label == "com.vendor.alfred.helper":
            return ["/opt/vendor-alfred/bin/agent-launch"]
        return []
...
def test_install_inventory_ignores_old_alfred_launcher_without_alfred_label(
    ...
    monkeypatch.delenv("ALFRED_REPO", raising=False)
    monkeypatch.setenv("WORKSPACE_ROOT", str(tmp_path / "missing-workspace"))
    monkeypatch.setenv("ALFRED_SETUP_LAUNCHD_LIST_FIXTURE", "com.vendor.worker\n")

    def fake_program_args(label: str, _env: dict[str, str]) -> list[str]:
        if label == "com.vendor.worker":
            return ["/opt/internal-alfred/bin/agent-launch"]
        return []
...
def test_install_inventory_blocks_unreadable_plist_with_old_alfred_launcher(
    ...
    launch_agents = home / "Library" / "LaunchAgents"
    launch_agents.mkdir(parents=True)
    (launch_agents / "old.alfred.worker.plist").write_text(
        """<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN"
 "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
  <key>Label</key>
  <string>old.alfred.worker</string>
  <key>ProgramArguments</key>
  <array>
    <string>{agent_launch}</string>
...
""".format(agent_launch=old_runtime / "bin" / "agent-launch"),
    ...
    assert inventory["unmanaged_scheduler_jobs"] == ["old.alfred.worker (unreadable)"]
    assert inventory["unmanaged_scheduler_count"] == 1
...
def test_install_inventory_prefers_active_systemd_unit_over_stale_service_file(
    ...
    def fake_run(args: list[str], **_kwargs: object) -> SimpleNamespace:
        ...
        if "com.example.worker.service" in args:
            return SimpleNamespace(
                returncode=0,
                stdout=f"{{ path={runtime / 'bin' / 'agent-launch'} ; argv[]={runtime / 'bin' / 'agent-launch'} lucius.py ; ignore_errors=no ; }}\n",
            )
        return SimpleNamespace(returncode=1, stdout="")
...
def test_install_inventory_ignores_stale_systemd_service_when_active_unit_clean(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
    ...
    def fake_run(*_args: object, **_kwargs: object) -> SimpleNamespace:
        return SimpleNamespace(returncode=0, stdout="/usr/bin/true\n")
...
def test_install_inventory_uses_systemd_service_label_identity(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
...
def test_install_inventory_blocks_generic_systemd_timer_with_unreadable_alfred_service(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
...
def test_install_inventory_ignores_systemd_timer_for_non_service_unit(
    ...
    def fake_run(args: list[str], **_kwargs: object) -> SimpleNamespace:
        if "vendor.cleanup.timer" in args:
            return SimpleNamespace(returncode=0, stdout="backup.target\n")
        if any(unit.endswith(".service") for unit in args):
            raise AssertionError("non-service timer units must not probe a service")
        return SimpleNamespace(returncode=1, stdout="")
...
def test_install_inventory_blocks_when_systemd_timer_unit_lookup_fails(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
...
def test_install_inventory_blocks_alfred_timer_when_unit_lookup_fails_despite_timer_file(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
    ...
    systemd_user = home / ".config" / "systemd" / "user"
    systemd_user.mkdir(parents=True)
    (systemd_user / "old-alfred.timer").write_text(
        "[Timer]\nUnit=backup.service\n",
        encoding="utf-8",
    )
    monkeypatch.setattr(setup_mod.os, "uname", lambda: SimpleNamespace(sysname="Linux"))
    ...
    def fake_run(args: list[str], **_kwargs: object) -> SimpleNamespace:
        ...
        if any(unit.endswith(".service") for unit in args):
            raise AssertionError(
                "Alfred-looking timer lookup failures must not trust disk fallback"
            )
        return SimpleNamespace(returncode=1, stdout="")
...
def test_install_inventory_defaults_systemd_timer_file_without_unit_to_same_name_service(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
    ...
    systemd_user = home / ".config" / "systemd" / "user"
    systemd_user.mkdir(parents=True)
    (systemd_user / "com.example.worker.timer").write_text(
        "[Timer]\nOnCalendar=hourly\n",
        encoding="utf-8",
    )
    monkeypatch.setattr(setup_mod.os, "uname", lambda: SimpleNamespace(sysname="Linux"))
...
def test_install_inventory_detects_systemd_shell_wrapper_launcher(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
    ...
    def fake_run(args: list[str], **_kwargs: object) -> SimpleNamespace:
        ...
        if "old.alfred.service" in args:
            return SimpleNamespace(
                ...
                stdout=(
                    "{ path=/bin/sh ; "
                    f"argv[]=/bin/sh -c 'exec {old_runtime / 'bin' / 'agent-launch'} lucius.py' ; "
                    "ignore_errors=no ; }\n"
                ),
            )
        return SimpleNamespace(returncode=1, stdout="")
...
def test_install_inventory_preserves_systemd_wrapper_semicolons(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
    ...
    def fake_run(args: list[str], **_kwargs: object) -> SimpleNamespace:
        ...
        if "old.alfred.service" in args:
            return SimpleNamespace(
                ...
                stdout=(
                    "{ path=/bin/sh ; "
                    f"argv[]=/bin/sh -c 'cd /tmp; exec {old_runtime / 'bin' / 'agent-launch'} lucius.py' ; "
                    "ignore_errors=no ; }\n"
                ),
            )
        return SimpleNamespace(returncode=1, stdout="")
