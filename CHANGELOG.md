...
- Live Claude + Codex usage in the native client. A local-state usage reader (`lib/server/usage.py`, exposed at `GET /api/usage` and `GET /api/usage/providers` and via the `alfred usage` CLI / `bin/alfred-usage.py`) reports subscription headroom from the Claude Code and Codex CLI state on the host, not the API list-price of tokens (which is meaningless under a Max/Pro subscription). The desktop Inbox capacity rail renders the current Claude and Codex headroom inline; a window the local state cannot confirm reads "not synced" rather than a fabricated number.
- Cinematic agent roster (default) with a dense-list toggle. The Agents view opens on a themed card command deck: one card per agent carrying its accent, status, cadence, runs-today, latest signal, and a monogram identity mark. A Cinematic / List toggle (persisted to `localStorage`) keeps the compact rail one click away. Hover lift and entrance motion respect `prefers-reduced-motion`, and the cards are real buttons so screen readers announce them as actionable controls.
- Step-level run events. `lib/agent_runner/agent_events.py` emits real lifecycle steps (`repo_picked`, `plan_created`, `pre_push_checks_passed`, `branch_pushed`, and more) at the points they actually happen in `lucius.py` / `batman.py`, each with an operator-visible detail string, so the client timeline tells the firing's story instead of only its start and stop. Progress is never fabricated.
...
- `clients/desktop`: first Tauri Mac/Linux client. It wraps the local `alfred serve` JSON APIs with Home, Compose, Fleet, Logs, and Setup gear surfaces; uses the Alfred brand fonts and logo; keeps local plan and firing detail in native inspector panes; opens explicit Slack/GitHub links outside the app; and restricts native API reads to localhost Alfred endpoints.
...
- `clients/desktop`: the native client now keeps local plan and firing details in app-side inspector panes instead of opening `alfred serve` detail pages in a browser. The Home decision queue groups repeated reliability signals, planning drafts, and memory candidates into action queues; the primary nav adapts between full labels, icons, and a second-row tab strip so smaller windows do not require horizontal scrolling. Setup has a local server URL field for custom ports, while upgraded installs pinned to the legacy `7000` default now probe the preferred `7010` runtime automatically.
