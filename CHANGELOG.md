...
- Slack memory curation: trusted users can run `memory`, `remember [repo:] <lesson>`, and `memory redis` from Slack to inspect review queues and stage candidates. Operator-only `memory promote <id>` / `memory reject <id>` decide what enters future recall, and `memory sync` previews Redis AMS sync before the explicit `memory sync now` write path.
...
- `clients/desktop`: first Tauri Mac/Linux client. It wraps the local `alfred serve` JSON APIs with Home, Compose, Fleet, Logs, and Setup gear surfaces; uses the Alfred brand fonts and logo; opens Slack/GitHub/local links outside the app; and restricts native API reads to localhost Alfred endpoints.
