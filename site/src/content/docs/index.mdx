---
title: Alfred
description: Run AI agents as repo teammates. Alfred turns GitHub issues, specs, and PR feedback into scheduled local Claude Code or Codex runs.
template: splash
head:
  - tag: title
    content: "Alfred: local AI agents for real repos"
hero:
  tagline: Turn GitHub issues, specs, and PR feedback into autonomous Claude Code or Codex runs from a machine you control.
  image:
...
A coding agent can complete a prompt. Recurring engineering work needs more around the prompt: a repo scope, a spec or issue, labels that show state, an isolated worktree, review rules, and a place where the result lands.

Alfred supplies that runtime and coordination layer. Agents coordinate through GitHub, work in clean local worktrees, report back to Slack, and can be routed to different engines by role. One agent can build with Claude Code while another reviews with Codex.

The default fleet is intentionally narrow: Batman plans multi-repo work, Lucius implements scoped issues, Ras al Ghul reviews PRs, Bane adds tests, and Nightwing can pick up unresolved review comments.
...
<CardGrid stagger>
  ...
  <Card title="Repo-teammate workflow" icon="document">
  ...
  <Card title="Multi-repo planning" icon="puzzle">
    Batman drafts rollout plans across repos from labelled GitHub issues, then stops before execution.
  </Card>
