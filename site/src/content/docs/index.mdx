---
title: Alfred
description: A local agent-fleet runtime for solo builders. AI development agents scheduled by launchd or systemd, on one machine you own.
template: splash
head:
  - tag: title
...
Alfred is at v0.2.1. It is usable today as a local engineering-agent fleet for one operator: install, starter setup, prompt seeding, GitHub label setup, launchd/systemd deployment, doctor, dry-run, Slack reporting, and Claude/Codex engine routing.
