...
Alfred-OS picks a different shape: every agent is a **fresh subprocess in its own git worktree**, dispatched by **`launchd`**, isolated by **per-agent IAM**, bounded by **per-day spend caps** with a **fleet-wide rate-limit poison pill**.
...
You're **not** building a multi-tenant SaaS, hosted orchestrator, or chat-first agent UX. Alfred-OS is deliberately small.
...
<LinkCard
  ...
  description="Why cron, why worktrees, why per-agent IAM. The design constraints that make alfred-os opinionated."
  href="/alfred-os/concepts/architecture/"
/>

## Status

Alfred-OS is at **v0.1.0** — initial public extraction from a fleet that has been running unattended for several months. The framework substrate (`agent_runner`) is stable for the operator's own use; expect rough edges if you fork.
