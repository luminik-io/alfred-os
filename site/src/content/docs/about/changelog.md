...
- **Slack memory curation**: trusted users can run `memory`, `memory harvest`, `remember [repo:] <lesson>`, and `memory redis` from Slack to inspect and stage memory, while operator-only `memory promote <id>` / `memory reject <id>` control what enters future recall.
...
- **Damian + Batman planning/execution**: Damian files `agent:bundle:<slug>` siblings across affected repos; Batman now executes approved plans (state machine through Slack reaction, label transition, or env override).
