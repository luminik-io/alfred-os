...
- **Redis-backed memory**: Alfred now defaults to `ALFRED_MEMORY_PROVIDERS=redis,fleet`, checks the local Agent Memory Server with `alfred brain ams-status`, and can carry older reviewed FleetBrain lessons into Redis with `alfred brain redis-sync`.
- **Slack memory curation**: trusted users can run `memory`, `memory harvest`, `remember [repo:] <lesson>`, `memory remember ...`, and `memory redis` from Slack to inspect and stage memory candidates. The configured approver decides what enters future recall.
...
- **FleetBrain local ledger**: review candidates, failure history, worker heartbeats, GitHub cache, and local reliability tooling. Redis Agent Memory is now the default recalled-lesson store; FleetBrain remains the local evidence and review layer. See [Memory providers](https://github.com/luminik-io/alfred-os/blob/main/docs/MEMORY_PROVIDERS.md).
