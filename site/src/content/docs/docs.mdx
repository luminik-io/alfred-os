...
<CardGrid stagger>
  ...
  <Card title="Batman: architect for multi-repo work" icon="puzzle">
    Batman holds a whole feature in one plan. It reads one `agent:large-feature` issue, drafts a rollout across every affected repo or monorepo package, posts the plan to Slack for approval, and files scoped child issues so Lucius can pick them up in parallel.
  </Card>
