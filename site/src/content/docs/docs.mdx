---
title: Alfred
description: Alfred is a local fleet of coding agents that turns specs and GitHub issues into pull requests, reviews, safe merges, and Slack reports while you are away.
template: splash
head:
  - tag: title
    content: "Alfred: coding agents that ship from your specs"
hero:
  tagline: "Coding agents that ship from your specs while you are away. Alfred plans across repos or monorepo packages, implements with your Claude Code and Codex subscriptions, opens pull requests, routes review, and reports to Slack."
  image:
...
That work needs durable coordination: specs that live outside the chat transcript, scoped GitHub issues, per-firing worktree isolation, role-based engine routing across Claude Code and Codex, review handoff, hard spend caps, and a state machine that keeps multiple agents from colliding. It runs on a host you already own and uses CLI auth you already pay for.
...
<CardGrid stagger>
  ...
  <Card title="Spec-to-PR workflow" icon="document">
    Specs become bounded jobs: plan, claim, open a worktree, implement, PR, review, test, report, and merge when your policy allows it.
  </Card>
