---
title: Alfred
description: Alfred keeps approved GitHub work moving while you are away. It plans across repos or monorepo packages, implements with the Claude Code and Codex subscriptions you already pay for, opens pull requests, routes review, and reports to Slack.
template: splash
head:
  - tag: title
    content: "Alfred: coding agents that work while you are away"
hero:
  tagline: "Autonomous coding agents for approved GitHub work. Alfred plans across repos or monorepo packages, implements with your Claude Code and Codex subscriptions, opens pull requests, routes review, and reports to Slack."
  image:
...
Interactive coding agents finish a prompt while you sit at the keyboard. Alfred is built for approved engineering work that should keep moving while you are away: planned features, follow-up tests, reviewer comments, dependency bumps, docs gaps, multi-repo rollouts.

That work needs durable coordination: scoped intake from GitHub issues and specs, per-firing worktree isolation, role-based engine routing across Claude Code and Codex, review handoff, hard spend caps, and a state machine that keeps multiple agents from colliding. It runs on a host you already own and uses CLI auth you already pay for.
