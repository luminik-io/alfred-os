...
export const GET: APIRoute = async ({ site }) => {
  ...
  const lines: string[] = [
    ...
    "Slack requests, rough plans, specs, and GitHub issues into PRs while the operator is away. It",
    "coordinates Claude Code and Codex CLI sessions on a",
    "machine the operator already owns: macOS via launchd, Linux via",
    "systemd. Each agent is a named role (Drake is the planner, Lucius",
    "is the senior developer, Ras al Ghul is the reviewer, Batman is the architect",
    "for multi-repo rollouts) with its own prompt, schedule, and label",
    "discipline.",
    "",
    ...
    "built for engineering work that should keep moving without the operator",
    ...
    "  child issues when the work is ready; otherwise it pings the operator",
    "  for the gaps.",
    ...
    "- Batman acts as the architect for multi-repo work via",
