...
function UsageBody({
  ...
  // Compact (e.g. the Inbox rail) is the headroom-at-a-glance view the operator
  // asked for: Claude 5h, Claude weekly, and Codex quota (when reported). Local
  // token evidence and the shipped count are full-view only -- shipped already
  // lives in the Inbox Shift card, so repeating it here would be noise.
  const visibleTiles = compact
    ? tiles.filter(
        (tile) =>
          tile.key === "five-hour" ||
          tile.key === "weekly" ||
          tile.key === "codex-quota",
      )
    : tiles;
  return (
    <>
