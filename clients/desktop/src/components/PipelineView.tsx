...
export function PipelineView({
  ...
  const loadError = state === "error" ? error || "Pipeline refresh failed." : null;
  ...
  return (
    <section className="alfred-pipeline" aria-label="Pipeline">
      <section className="alfred-page-hero px-4 py-4" aria-label="Pipeline summary">
      ...
      {!loading && !hasAnything && !hardError ? (
        ...
        <div className="alfred-pipeline__columns">
      ...
      >
        <SheetContent className="plan-detail-sheet">
          <SheetHeader>
            <SheetTitle>
              {selectedPlan ? "Review plan" : "Pipeline item"}
            </SheetTitle>
            <SheetDescription>
              {selectedPlan
...
function PipelineColumn({
  ...
  return (
    <section className="alfred-pipeline__column" aria-label={`${label} (${count})`}>
      ...
      <div className="alfred-pipeline__cards">{children}</div>
    </section>
  );
}
