...
describe("OnboardingView seven-step takeover", () => {
  ...
  it("does not save the fallback roster after server hydration fails", async () => {
    vi.spyOn(api, "loadSetupStatus").mockResolvedValue(
      makeStatus({
        repos: { selected: ["octocat/web"], count: 1, keys: ["ALFRED_QUEUE_REPOS", "ALFRED_SHIPPED_REPOS"] },
      }),
    ...
    expect(screen.getAllByText(/could not load saved fleet names/i).length).toBeGreaterThan(0);
  });
  ...
  it("resets the Fleet save acknowledgement when the runtime changes", async () => {
    ...
    view.rerender(<OnboardingView {...props} baseUrl="http://127.0.0.1:7020" />);
    await user.click(within(stepper).getByRole("button", { name: /^first request$/i }));
  ...
  it("resets the Fleet save acknowledgement when the same runtime reconnects", async () => {
    vi.spyOn(api, "loadSetupStatus").mockResolvedValue(
      makeStatus({
        repos: { selected: ["octocat/web"], count: 1, keys: ["ALFRED_QUEUE_REPOS", "ALFRED_SHIPPED_REPOS"] },
      }),
  ...
  it("routes Slack Skip back to Fleet after reconnect clears its acknowledgement", async () => {
    vi.spyOn(api, "loadSetupStatus").mockResolvedValue(
      makeStatus({
        repos: { selected: ["octocat/web"], count: 1, keys: ["ALFRED_QUEUE_REPOS", "ALFRED_SHIPPED_REPOS"] },
      }),
  ...
  it("requires saving a non-default fleet choice in this onboarding session", async () => {
    ...
    await waitFor(() => expect(onRosterThemeChange).toHaveBeenCalledWith("transformers"));
  });

  it("lets the browser preview advance past Fleet without saving", async () => {
    vi.spyOn(api, "supportsNativeActions").mockReturnValue(false);
    vi.spyOn(api, "loadSetupStatus").mockResolvedValue(
      makeStatus({
        repos: { selected: ["octocat/web"], count: 1, keys: ["ALFRED_QUEUE_REPOS", "ALFRED_SHIPPED_REPOS"] },
